<?php 
	...
	class Page{
		...
		public static function makePage($rowCount,$pageSize){
			...
			$limtStr = " limit ".($page-1)*$pageSize." , ".$pageSize." ";   //limit 5,10
			...
			if($pages>0){
				...
				}else if ($page>=6 && $page<$pages-3) {   //47
					//重新计算开始和结束的页码
					$start = $page-3;
					$end = $page+2;
					//拼接页面1、2
				...
				//如果当前页小于总页数，显示下一页和末页
				if($page<$pages){
					$pageStr .="<a href='?{$old}page=".($page+1)."'>下一页</a>";
					$pageStr .="<a href='?{$old}page=".$pages."'>末页</a>";
				}

				$pageStr .= "<span>共<b>{$pages}</b>页到第</span>";
				$pageStr .= "<form action='' style='display:inline-block;'><input type='text' class='txt' name='page' value='{$page}'>";
				$pageStr .= "<span>页</span>";
				$pageStr .= $inputStr;
